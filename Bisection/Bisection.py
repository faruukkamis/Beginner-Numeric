...
def Bisection(f,p1,p2,TOL,iteration):
    ...
    # f(m) is used instead of whichever function has the same sign. 
        ...
        return Bisection(f, m, p2, TOL, iteration)
    
    elif np.sign(f(m)) == np.sign(f(p2)):
        return Bisection(f, p1, m, TOL, iteration)

print(' The root is : ', Bisection(f, p1, p2, TOL, iteration))
